...
REGISTER_OP("PrimitiveCoverageSplitLossV3")
.Input("in_z: float")
.Input("in_q: float")
.Input("in_t: float")
.Input("in_pos: float")
...
.Doc(R"doc(
Output each cube's total coverage distance summation, each cube contains
different number of points.
)doc");

class PrimitiveCoverageSplitLossV3Op : public OpKernel {
 public:
  explicit PrimitiveCoverageSplitLossV3Op(OpKernelConstruction* context)
      :  OpKernel(context) {}
  ...
  void Compute(OpKernelContext* context) override {
    ...
    compute_coverage_split_loss_v3(context, batch_size_, n_cube_, n_point_,
...
REGISTER_KERNEL_BUILDER(Name("PrimitiveCoverageSplitLossV3").Device(DEVICE_GPU),
    PrimitiveCoverageSplitLossV3Op);


REGISTER_OP("PrimitiveCoverageSplitLossV3Grad")
...
.Doc(R"doc(
Gradient for coverage split loss.
)doc");

class PrimitiveCoverageSplitLossV3GradOp : public OpKernel {
 public:
  explicit PrimitiveCoverageSplitLossV3GradOp(OpKernelConstruction* context)
      : OpKernel(context) {}
  ...
  void Compute(OpKernelContext* context) override {
    ...
    compute_coverage_split_loss_v3_grad(context, n_cube_, n_point_, batch_size_,
...
REGISTER_KERNEL_BUILDER(Name("PrimitiveCoverageSplitLossV3Grad").Device(DEVICE_GPU),
    PrimitiveCoverageSplitLossV3GradOp);
