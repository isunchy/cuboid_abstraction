...
REGISTER_OP("PrimitiveMutexLossV3")
.Input("in_z: float")
.Input("in_q: float")
.Input("in_t: float")
...
.Doc(R"doc(
Sample points in cube volume, compute the distance of each point invades the
corresponding cube, except its original cube.
)doc");

class PrimitiveMutexLossV3Op : public OpKernel {
 public:
  explicit PrimitiveMutexLossV3Op(OpKernelConstruction* context)
      : OpKernel(context) {
  ...
  void Compute(OpKernelContext* context) override {
    ...
    // compute mutex loss
    compute_mutex_loss_v3(context, n_cube_, batch_size_, scale_, in_z_ptr,
...
REGISTER_KERNEL_BUILDER(Name("PrimitiveMutexLossV3").Device(DEVICE_GPU),
    PrimitiveMutexLossV3Op);


REGISTER_OP("PrimitiveMutexLossV3Grad")
...
class PrimitiveMutexLossV3GradOp : public OpKernel {
 public:
  explicit PrimitiveMutexLossV3GradOp(OpKernelConstruction* context)
      : OpKernel(context) {
    OP_REQUIRES_OK(context, context->GetAttr("scale", &scale_));
  }
  ...
  void Compute(OpKernelContext* context) override {
    ...
    compute_mutex_loss_grad_v3(context, n_cube_, batch_size_, scale_,
...
REGISTER_KERNEL_BUILDER(Name("PrimitiveMutexLossV3Grad").Device(DEVICE_GPU),
    PrimitiveMutexLossV3GradOp);
